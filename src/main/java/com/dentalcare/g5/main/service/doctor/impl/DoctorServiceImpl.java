...
public class DoctorServiceImpl implements DoctorService {
    ...
    public DoctorDto addDoctor(DoctorCreateRequest payload) {
        ...
        if (payload.getEspecialidadId() == null) {
            throw new RuntimeException("El ID de especialidad es requerido");
        }
        if (payload.getUsuario() == null || payload.getUsuario().getEmail() == null) {
            throw new RuntimeException("La información del usuario y el email son requeridos");
        }
        ...
        try {
            ...
            Especialidad especialidad = especialidadRepository.findById(payload.getEspecialidadId())
                    .orElseThrow(() -> new RuntimeException("Especialidad no encontrada con ID: " + payload.getEspecialidadId()));
            ...
            throw new RuntimeException("No se pudo crear el doctor debido a una violación de restricción", e);
        }
    }
    ...
    public DoctorDto updateDoctor(DoctorUpdateRequest payload) {
        if (payload.getId() == null) {
            throw new RuntimeException("El ID del doctor es requerido para la actualización");
        }
    
        Doctor existingDoctor = doctorRepository.findById(payload.getId())
                .orElseThrow(() -> new RuntimeException("Doctor no encontrado con ID: " + payload.getId()));
        
        try {
            // Actualizar información del usuario
            if (payload.getNombre() != null || payload.getApellido() != null || 
                payload.getTelefono() != null) {
                
                Usuario usuario = existingDoctor.getUsuario();
                if (usuario == null) {
                    throw new RuntimeException("El doctor no tiene un usuario asociado");
                }
                
                if (payload.getNombre() != null) usuario.setNombre(payload.getNombre());
                if (payload.getApellido() != null) usuario.setApellido(payload.getApellido());
                if (payload.getTelefono() != null) usuario.setTelefono(payload.getTelefono());
            ...
            throw new RuntimeException("No se pudo actualizar el doctor debido a una violación de restricción", e);
        }
    }
